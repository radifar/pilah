...
def display_removed_residues(residue_filter):
    if residue_filter.res_w_missing_atoms:
        ...
        for (
            ...
            residue_name,
            residue_number,
        ) in residue_filter.res_w_missing_atoms:
            console.print(
                f"[deep_pink1]         {chain_id}       {residue_name}          {residue_number}[/deep_pink1]"
            )
    if residue_filter.res_w_incorrect_bond_length_angle:
        ...
        for (
            ...
            residue_name,
            residue_number,
        ) in residue_filter.res_w_incorrect_bond_length_angle:
            console.print(
                f"[deep_pink1]         {chain_id}       {residue_name}          {residue_number}[/deep_pink1]"
            )
...
def extract(data):
    ...
    if any(list(total_insertion.values())):
        ...
        console.print(
            "[bold deep_pink1]     and the residues following those residues will be renumbered.[/bold deep_pink1]"
        )
        console.print(
            "[bold deep_pink1]     The mapping between original residue number and the new one will be logged into the Log file[/bold deep_pink1]"
        )
    ...
    if total_ligand > 1:
        ...
        if res_num_not_provided:
            ...
            console.print(
                "[bold deep_pink1]     To choose ligand with specific residue number use 'ligand_res_num' option.[/bold deep_pink1]"
            )
            console.print(
                f"[bold deep_pink1]     The residue number of ligand detected in chain {ligand_chain} are: {ligand_res_num}.[/bold deep_pink1]"
            )
