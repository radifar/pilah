...
Few thing to note, all the format is originally derived from PDB that is processed with Biopython and RDKit. The SDF format is also generated using RDKit. MOL2 format is generated using Openbabel. And PDBQT is generated using Meeko.
...
PiLAH can only extract the ligand and the protein from one chain and unable to select multiple chain at once.
By default PiLAH will select chain A, but you can choose other chain by assigning the value to option `chain` like so:
